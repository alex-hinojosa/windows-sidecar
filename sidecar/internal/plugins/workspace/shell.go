...
func getTmuxInstallInstructions() string {
	...
	case "windows":
		return "winget install psmux  # or install tmux with MSYS2/Cygwin"
	default:
		return "Install psmux or tmux from your package manager"
	}
}
...
func (p *Plugin) createNewShell(customName string) tea.Cmd {
	if !isTmuxInstalled() {
		return func() tea.Msg {
			return ShellCreatedMsg{Err: fmt.Errorf("psmux/tmux not installed: %s", getTmuxInstallInstructions())}
		}
	}
