...
func (p *Plugin) renderWelcomeGuide(width, height int) string {
	...
	if !isTmuxInstalled() {
		lines = append(lines, warningStyle.Render("⚠ psmux/tmux Required"))
		lines = append(lines, "")
		lines = append(lines, dimText("Workspaces and shell sessions require psmux or tmux to be installed."))
		lines = append(lines, "")
		lines = append(lines, sectionStyle.Render("Install psmux/tmux:"))
	...
	// Title
	title := lipgloss.NewStyle().Bold(true).Render("psmux/tmux Quick Reference")
	...
	lines = append(lines, "")
	lines = append(lines, dimText("psmux uses tmux-compatible prefix and session commands"))
...
func (p *Plugin) renderOrphanedMessage(agentType AgentType) string {
	...
	lines = append(lines, "")
	lines = append(lines, dimText("The mux session has ended, but your worktree and work are still intact."))
	lines = append(lines, "")
...
func (p *Plugin) renderShellPrimer(width, height int) string {
	...
	if !isTmuxInstalled() {
		lines = append(lines, warningStyle.Render("⚠ psmux/tmux Required"))
		lines = append(lines, "")
		lines = append(lines, dimText("The project shell requires psmux or tmux to be installed."))
		lines = append(lines, "")
		lines = append(lines, sectionStyle.Render("Install psmux/tmux:"))
		lines = append(lines, dimText("  "+getTmuxInstallInstructions()))
		lines = append(lines, "")
	...
	// Description
	lines = append(lines, dimText("A psmux/tmux session in your project directory for running"))
