...
const (
	paneIDLookupAttempts     = 10
	paneIDLookupInitialDelay = 10 * time.Millisecond
	paneIDLookupMaxDelay     = 200 * time.Millisecond
)
...
func firstPaneID(output string) string {
...
func lookupPaneIDOnce(sessionName string) string {
	cmd := exec.Command("tmux", "list-panes", "-t", sessionName, "-F", "#{pane_id}")
	output, err := cmd.Output()
	if err != nil {
		return ""
	}
	return firstPaneID(string(output))
}
...
func getPaneID(sessionName string) string {
	...
	delay := paneIDLookupInitialDelay
	for attempt := 0; attempt < paneIDLookupAttempts; attempt++ {
		if paneID := lookupPaneIDOnce(sessionName); paneID != "" {
			globalPaneIDCache.set(sessionName, paneID)
			return paneID
		}
		if attempt == paneIDLookupAttempts-1 {
			break
		}
		time.Sleep(delay)
		delay *= 2
		if delay > paneIDLookupMaxDelay {
			delay = paneIDLookupMaxDelay
		}
	}
	return ""
}
