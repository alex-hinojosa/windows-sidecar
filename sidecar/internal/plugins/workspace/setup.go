...
const (
	...
	setupPowerShellScriptName = ".worktree-setup.ps1"
)
...
func (p *Plugin) runSetupScript(worktreePath, branchName string) error {
	scriptPath, command, args, ok, err := p.resolveSetupScript()
	if err != nil {
		return err
	}
	if !ok {
...
func (p *Plugin) resolveSetupScript() (scriptPath, command string, args []string, ok bool, err error) {
	shPath := filepath.Join(p.ctx.WorkDir, setupScriptName)

	if runtime.GOOS == "windows" {
		psPath := filepath.Join(p.ctx.WorkDir, setupPowerShellScriptName)
		if _, statErr := os.Stat(psPath); statErr == nil {
			psExe, lookErr := exec.LookPath("pwsh")
			if lookErr != nil {
				psExe, lookErr = exec.LookPath("powershell")
			}
			if lookErr != nil {
				return psPath, "", nil, false, lookErr
			}
			return psPath, psExe, []string{"-NoProfile", "-ExecutionPolicy", "Bypass", "-File", psPath}, true, nil
		} else if !os.IsNotExist(statErr) {
			return psPath, "", nil, false, statErr
		}
	}
	...
	if _, statErr := os.Stat(shPath); os.IsNotExist(statErr) {
		...
		return shPath, "", nil, false, statErr
	}

	bashExe, lookErr := exec.LookPath("bash")
	if lookErr != nil {
		return shPath, "", nil, false, lookErr
	}
	return shPath, bashExe, []string{shPath}, true, nil
}
