...
func detectPiSessionStatus(worktreePath string) (WorktreeStatus, bool) {
	...
	projectDir := filepath.Join(home, ".pi", "agent", "sessions", piProjectDirName(absPath))
...
func piProjectDirName(absPath string) string {
	path := strings.TrimPrefix(filepath.ToSlash(absPath), "/")
	var b strings.Builder
	for _, r := range path {
...
func getAmpThreadStatus(threadPath, worktreePath string) (WorktreeStatus, bool) {
	...
	for _, tree := range thread.Env.Initial.Trees {
		if cwdMatches(tree.URI, worktreePath) {
			pathMatches = true
			break
		}
	}
...
func cwdMatches(cwd, worktreePath string) bool {
	cwd = normalizeSessionPath(cwd)
	worktreePath = normalizeSessionPath(worktreePath)
	if runtime.GOOS == "windows" {
		return strings.EqualFold(cwd, worktreePath) ||
			strings.HasPrefix(strings.ToLower(cwd), strings.ToLower(worktreePath+string(filepath.Separator)))
	}
