...
func TestParsePaneSize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotWidth, gotHeight, gotOK := parsePaneSize(tt.output)
			if gotWidth != tt.wantWidth || gotHeight != tt.wantHeight || gotOK != tt.wantOK {
				t.Errorf("parsePaneSize(%q) = (%d, %d, %v), want (%d, %d, %v)", tt.output, gotWidth, gotHeight, gotOK, tt.wantWidth, tt.wantHeight, tt.wantOK)
			}
		})
	}
}
