...
const (
	paneSizeQueryAttempts     = 5
	paneSizeQueryInitialDelay = 25 * time.Millisecond
	paneSizeQueryMaxDelay     = 100 * time.Millisecond
)
...
func parsePaneSize(output string) (width, height int, ok bool) {
...
func QueryPaneSize(target string) (width, height int, ok bool) {
	...
	delay := paneSizeQueryInitialDelay
	for attempt := 0; attempt < paneSizeQueryAttempts; attempt++ {
		cmd := exec.Command("tmux", "display-message", "-t", target, "-p", "#{pane_width},#{pane_height}")
		output, err := cmd.Output()
		if err == nil {
			if width, height, ok := parsePaneSize(string(output)); ok {
				return width, height, true
			}
		}
		if attempt == paneSizeQueryAttempts-1 {
			break
		}
		time.Sleep(delay)
		delay *= 2
		if delay > paneSizeQueryMaxDelay {
			delay = paneSizeQueryMaxDelay
		}
	}
	return 0, 0, false
}
